...
% Use the styles in definitions of proof, theorem, etc.
...
\begin{document}
  ...
  We note that systems with transmission zeros cannot be squared up using the method as described in Reference [\citen{misra.squareup.1992}], which has led to a recent modification to overcome this limitation and allow the design of output feedback controllers for systems with stable transmission zeros\cite{quwiese.ifac.2014}.
  ...
  \begin{rem-dan}
    ...
    The case of square systems has been given in Reference [\citen{huang.designspr.1999}] and is discussed in Section~\ref{sec.adaptivecontroldesign}.
  \end{rem-dan}
  ...
  A controller along the lines of References [\citen{lavretsky.output.2010,wise.obltrdesign.2013,qu.gnc.2013}] is proposed, as it leads to a low order robust controller.
  ...
  \begin{proof-dan}
    The proof can be found in Reference [\citen{narendra.frequency.1973}].
  \end{proof-dan}
  ...
  \begin{proof-dan}
    The proof can be found in Reference [\citen{huang.designspr.1999}].
  \end{proof-dan}
  ...
  \begin{proof-dan}
    The proof can be found in Reference [\citen{boyd.lmibook.1994}].
  \end{proof-dan}
  ...
  Reference [\citen{huang.designspr.1999}] gave the inequality\ \eqref{eqn.NAMlyap} for a square system, suggesting that $X$ be obtained by solving this LMI numerically.
  However, it was shown in Reference [\citen{kouvaritakis.part1.1976}] that for a square system, the eigenvalues of $NAM$ are the transmission zeros of the system and the annihilators $N$ and $M$ can be always be selected such that $NM=I$.
  ...
  \begin{defn-dan}[(Markov Parameters)]
    [\citen{antsaklis.linearsystems.2006}]\label{defn.markov}
  ...
  \begin{proof-dan}
    The proof can be found in Reference [\citen{antsaklis.linearsystems.2006}].
  \end{proof-dan}
  ...
  \begin{lem-dan}\label{lem.E}
    Reference [\citen{narendra.stable.2005}]
  ...
  \begin{proof-dan}
    The proof can be found in Reference [\citen{chen.introduction.1970}]
  \end{proof-dan}
  ...
  \begin{proof-dan}
    ...
    In Reference [\citen{narendra.stable.2005}] it is shown that $E(W_{p}(s))=K_{p}$.
  \end{proof-dan}
  ...
  In Reference [\citen{wiese.adaptive.2013}] a \textit{state feedback} LQR baseline controller with integral action and augmented with an adaptive component was applied to design two independent CRM based state feedback adaptive controllers\textemdash{}one for each of the plant subsystems.
  ...
  \begin{figure}[H]
    \begin{center}
      \includegraphics[width=4.3in]{\figurepath/gnc2015_simulation_block.pdf}
      \vspace{-0.1in}
